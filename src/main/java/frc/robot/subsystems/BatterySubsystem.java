...
public class BatterySubsystem extends SubsystemBase {
  //Constants
  public static final double minVoltageYellow = 12.8;
  ...
  public BatterySubsystem() {
    ...
    Shuffleboard.getTab("Battery")
        .addBoolean("Voltage Red", () -> getVoltage() < minVoltageRed)
        .withPosition(2, 1)
    ...
    Shuffleboard.getTab("Battery")
        .addBoolean("HC Yellow", () -> getHighCurrentTimer() > timeInSecondsHighCurrentYellow)
        .withPosition(3, 3)
        .withProperties(Map.of("colorWhenFalse", "black"))
        .withProperties(Map.of("colorWhenTrue", "yellow"));
    // Adds voltage reader tab
    sbVoltage = btTab.add("Battery Voltage", 0)
        .withSize(2, 1)
        .withPosition(2, 0).getEntry();
    // Adds current reader tab
    sbInputCurrent =
        btTab.add("Battery Input Current", 0)
        .withSize(2, 1)
        .withPosition(4, 0).getEntry();
    // Adds sim voltage reader tab
    sbSimVoltage = btTab.add("Simulation Voltage", 0)
        .withSize(2, 1)
        .withPosition(0, 0).getEntry();
    // Adds general timer reader tab
    sbTimer = btTab.add("Timer", 0)
        .withSize(2, 1)
        .withPosition(0, 2).getEntry();
    // Adds high current timer reader tab
    sbTimerHighCurrent = btTab.add("High Current Timer", 0)
        .withSize(2, 1)
        .withPosition(0, 1).getEntry();
    // Adds timer change reader tab
    sbTimerChange = btTab.add("Change Timer", 0)
        .withSize(0, 1)
        .withPosition(4, 0).getEntry();
